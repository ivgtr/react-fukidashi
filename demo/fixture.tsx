...
function Fixture() {
  ...
  return <>
    <div style={{ position: 'fixed', top: 0, left: 0, zIndex: 5000 }}>
      ...
      <select aria-label="Placement" value={placement} onChange={event => setPlacement(event.target.value as Placement)}>
        {(['top', 'right', 'bottom', 'left'] as const).flatMap(side => [side, `${side}-start`, `${side}-end`]).map(value => <option key={value}>{value}</option>)}
      </select>
      <output aria-label="Exits">{exits}</output>
    </div>
    <Fukidashi open={open} placement={placement} motion="slide" duration={160} onExitComplete={() => setExits(value => value + 1)}
      anchorStyle={{ position: 'fixed', top: edge ? 2 : 330, left: edge ? 2 : 450 }}
      anchor={props => <button {...props} data-testid="anchor" style={{ width: 100, height: 40 }}>Anchor</button>}
      data-testid="bubble" style={{ '--fukidashi-width': '240px' }}>
