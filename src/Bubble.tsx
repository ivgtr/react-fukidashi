...
export const Bubble = forwardRef<HTMLDivElement, BubbleProps>(function Bubble({
  children, variant = 'soft', tail = true, className, style, contentClassName,
  contentStyle, tailRef, ...attributes
}, ref) {
  ...
  return (
    <div {...attributes} ref={ref} className={['fukidashi-bubble', className].filter(Boolean).join(' ')}
      data-variant={variant} style={variables}>
      {tail !== false && size > 0 && <span ref={tailRef} className="fukidashi-tail"
        data-side={options.side ?? 'bottom'} aria-hidden="true" />}
      <div className={['fukidashi-content', contentClassName].filter(Boolean).join(' ')} style={contentStyle}>
        {children}
      </div>
    </div>
  );
