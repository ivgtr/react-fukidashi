...
export const Fukidashi = forwardRef<HTMLDivElement, FukidashiProps>(function Fukidashi({
  anchor, children, open = true, placement = 'top', gap = 10, tail = true,
  portal = true, strategy = 'fixed', collisionPadding = 12, avoidCollisions = true,
  trackAnchor = false, motion = 'pop', duration = 180, reducedMotion = 'system',
  zIndex = 1000, anchorClassName, anchorStyle, onExitComplete, id: suppliedId,
  role = 'note', ...bubbleProps
}, ref) {
  ...
  const tailSize = tail === false ? 0 : nonNegative(typeof tail === 'object' ? tail.size ?? 12 : 12, 12);
  const padding = nonNegative(collisionPadding, 12);
  const middleware = useMemo(() => [
    offset((Number.isFinite(gap) ? gap : 10) + tailSize / Math.SQRT2),
    ...(avoidCollisions ? [flip({ padding }), shift({ padding }), size({ padding,
      apply({ availableWidth, availableHeight, elements }) {
        elements.floating.style.setProperty('--fukidashi-available-width', `${Math.max(0, availableWidth)}px`);
        elements.floating.style.setProperty('--fukidashi-available-height', `${Math.max(0, availableHeight)}px`);
      },
    })] : []),
  ...
  const { refs, floatingStyles, placement: resolvedPlacement, middlewareData, isPositioned } = useFloating({
    open: present, placement, strategy, middleware, whileElementsMounted: whileMounted,
  });
  ...
  const arrowOffset = (side === 'top' || side === 'bottom') ? middlewareData.arrow?.x : middlewareData.arrow?.y;
  ...
  const motionStyle: FukidashiStyle = {
    ...
    '--fukidashi-origin': side === 'top' ? `${center} bottom` : side === 'bottom' ? `${center} top`
  ...
  const floating = present ? (
    <div ref={refs.setFloating} className="fukidashi-positioner" data-placement={resolvedPlacement}
      aria-hidden={!open || undefined} style={{ ...floatingStyles, zIndex, visibility: isPositioned ? 'visible' : 'hidden' }}>
      <div className="fukidashi-motion" data-motion={motion} data-state={entered ? 'open' : 'closed'}
        data-reduced-motion={reducedMotion} style={motionStyle}>
        <Bubble {...bubbleProps} ref={ref} id={id} role={role} tailRef={tailRef}
  ...
  return <>
    <span ref={refs.setReference} className={['fukidashi-anchor', anchorClassName].filter(Boolean).join(' ')} style={anchorStyle}>
      {typeof anchor === 'function' ? anchor({ 'aria-describedby': open ? id : undefined }) : anchor}
    </span>
