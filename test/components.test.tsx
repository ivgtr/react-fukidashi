...
describe('composable surfaces', () => {
  it('forwards Bubble attributes, DOM refs and arbitrary content', () => {
    ...
    render(
      <Bubble
        ...
        variant="comic"
        tail={false}
        style={{ '--fukidashi-background': 'pink' }}
      >
  ...
  it.each(['top', 'right', 'bottom', 'left'] as const)('supports a standalone %s tail', (side) => {
    ...
    expect(
      container
        .querySelector<HTMLElement>('.fukidashi-bubble')
        ?.style.getPropertyValue('--fukidashi-tail-size'),
    ).toBe('18px');
  });
  it('exposes full accessible text once and imperative typewriter controls', () => {
    ...
    expect(container.querySelector('.fukidashi-typewriter-visible')?.childNodes.length).toBe(1);
    expect(container.firstElementChild?.getAttribute('data-visible-length')).toBe('0');
    expect(
  ...
  it('does not render a closed bubble', () => {
    render(
      <Fukidashi open={false} anchor={<button>Anchor</button>}>
        Closed message
      </Fukidashi>,
    );
